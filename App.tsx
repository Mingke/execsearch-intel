...
import { type AnalysisResult, AnalysisStatus, type HistoryItem } from "./types"
import { analyzeContent } from "./services/geminiService"
import { historyService } from "./services/historyService"
import { getSupabase, isSupabaseConfigured } from "./services/supabaseClient" // Import getSupabase and isSupabaseConfigured instead of direct supabase
      ...
      <div className="flex h-screen items-center justify-center bg-background text-foreground">
        <div className="flex flex-col items-center gap-4 max-w-md text-center px-4">
          ...
          <p className="text-sm text-muted-foreground">
            Supabase is not configured. Please add{" "}
            <code className="bg-muted px-1.5 py-0.5 rounded text-xs">NEXT_PUBLIC_SUPABASE_URL</code> to your environment
            variables.
          </p>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-background text-foreground font-sans flex flex-col transition-colors duration-300">
          ...
          <div className="flex items-center gap-2">
            ...
            {userCredits && (
              <div className="hidden sm:flex items-center gap-1.5 px-3 py-1 bg-muted/50 rounded-full border border-border mr-2">
                <div
                  className={`h-2 w-2 rounded-full ${userCredits.count >= userCredits.limit ? "bg-destructive" : "bg-green-500"}`}
                />
                <span className="text-xs font-medium tabular-nums">
                  Credits: {userCredits.count} / {userCredits.limit}
                </span>
              </div>
            )}
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-12 space-y-6 sm:space-y-12">
        ...
        {result && (
          <div
            ref={resultRef}
            className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300 scroll-mt-20"
          >
        ...
        {userRole === "admin" && <AdminDashboard isOpen={isAdminOpen} onClose={() => setIsAdminOpen(false)} />}
      </main>

      <footer className="w-full border-t border-border/40 py-6 mt-auto">
        <div className="container max-w-4xl mx-auto text-center px-4">
          <p className="text-xs text-muted-foreground">Executive Search Intelligence Tool &copy; 2025 MRS.ai.</p>
